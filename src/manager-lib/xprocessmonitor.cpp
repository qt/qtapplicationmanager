...
XProcessMonitor::XProcessMonitor(const QString &appId, QObject *parent)
    : QObject(parent)
...
XProcessMonitor::~XProcessMonitor()
{
    delete m_memoryMonitor;
}

bool XProcessMonitor::isMemoryReportingEnabled() const
{
    return m_memoryReportingEnabled;
}

void XProcessMonitor::setMemoryReportingEnabled(bool memoryReportingEnabled)
{
...
bool XProcessMonitor::isCpuLoadReportingEnabled() const
{
    return false;
}

void XProcessMonitor::setCpuLoadReportingEnabled(bool cpuReportingEnabled)
{
    Q_UNUSED(cpuReportingEnabled)
}

bool XProcessMonitor::isFpsReportingEnabled() const
{
    return false;
}

void XProcessMonitor::setFpsReportingEnabled(bool fpsReportingEnabled)
{
    Q_UNUSED(fpsReportingEnabled)
}

void XProcessMonitor::readData()
{
...
QAbstractListModel *XProcessMonitor::memoryMonitor()
{
    if (!m_memoryMonitor)
        return nullptr;
    else
...
void XProcessMonitor::obtainPid()
{
    if (m_appId.isEmpty())
