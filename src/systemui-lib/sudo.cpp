...
void Sudo::forkServer(DropPrivileges dropPrivileges)
{
    ...
    int socketFds[2];
    if (::socketpair(AF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0, socketFds) != 0)
    ...
    if (pid < 0) {
        ...
        ::close(socketFds[1]); // close the parent's end of the socketpair
        ::close(0);
        ::setsid();
    ...
    ::close(socketFds[0]); // close the child's end of the socketpair
    try {
        // reset umask
        if (realUmask)
            ::umask(realUmask);
